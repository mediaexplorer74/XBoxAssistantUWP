// *********************************************************
// Type: LRC.LocalSubnet.LRC_JOIN_SESSION_PARAMS
// Assembly: LRC.LocalSubnet, Version=3.2.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 67B18A68-32AE-4F0B-8110-A02EDA1EEA1C
// *********************************************************LRC.LocalSubnet.dll


namespace LRC.LocalSubnet
{
  public struct LRC_JOIN_SESSION_PARAMS
  {
    public uint SupportedProtocolVersion;
    public uint SessionSequenceNumber;
    public uint NotificationSequenceNumber;
  }
}
