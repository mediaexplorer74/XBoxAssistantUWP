// *********************************************************
// Type: Microsoft.Phone.Marketplace.Util.DeflaterManaged
...
namespace Microsoft.Phone.Marketplace.Util
{
  internal class DeflaterManaged : IDeflater, IDisposable
  {
    ...
    private bool NeedsInput() => ((IDeflater) this).NeedsInput();

    bool IDeflater.NeedsInput() => this.input.Count == 0 && this.deflateEncoder.BytesInHistory == 0;
    ...
    private bool UseCompressed(double ratio) => ratio <= 1.0;

    private void FlushInputWindows() => this.deflateEncoder.FlushInput();

    private void WriteFinal() => this.copyEncoder.GetBlock((DeflateInput) null, this.output, true);
